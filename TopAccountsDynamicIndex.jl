...
# ╔═╡ 2a1f210f-8126-4063-bed9-fe6b7e2d21a3
begin
	using Plots
	using Statistics
end

# ╔═╡ f868d34c-9128-428e-a635-e03c116079dd
md"""
# Parsiq Bounty
...
# ╔═╡ a733d71a-de92-4f39-99c7-3650459085c9
md"""
## 1.- Load the top accounts by ETH balance.
"""

# ╔═╡ c36e45ec-f317-488e-b405-33967e784412
begin
...
# ╔═╡ e3854b1e-8e62-4f1b-b187-69d2a22f8703
md"""
## 2.- Addresses to be excluded.
We are going to track addresses of big holders, 
so some of them have to be excluded:
...
# ╔═╡ 7a065ccb-692c-42b7-8d4d-ee4a871a36b2
begin 
	exAccounts = []
	open("/Volumes/ROCKET-XTRM/ParsiqBounty/eth_accounts_exreason.csv", "r") do io
	    while !eof(io)
	        rw = readline(io)
	        rw = split(rw,",")
...
# ╔═╡ e3f6f00c-50af-4154-8821-7f1c2f5106ea
...
# ╔═╡ 506f03bf-03a0-4f6f-a3f5-e6108ef349aa
begin 
	accounts = []
...
# ╔═╡ d9b569a6-b654-4cc8-8f53-9cf594bbac3f
accounts

# ╔═╡ 8528c65d-82a4-45c9-82b0-eb9aaa6d4d4a
begin 
...
# ╔═╡ dffe5c89-c615-45b6-bc1f-5cfae4ff74ba
accRank["0x73bceb1cd57c711feac4224d062b0f6ff338501e"]

# ╔═╡ b777ce23-0f0a-4928-8b46-5f8f4597d59b
accRank["0xce7d28b2a232004c0e102519fb2b50e6360d5840"]

# ╔═╡ db33d1e3-c880-4d1c-9031-bc3ca910b6e8
md"""
## 4.- Parsiq trigger.
...
# ╔═╡ d782eb50-f952-4bff-a36a-f7a97c832745
md"""
## 5.- Delivery Channel. Telegram.
Transfer from {from} to {to} for {value|eth} ETH detected, {txInfo.txHash}, {ts.blockts}, {cryptorankProvider_fiatRateUSD_transferAmount.value|cryptorankProvider_fiatRateUSD_transferAmount.decimals} USD (1 {assetSymbol} = {cryptorankProvider_fiatRateUSD.value|cryptorankProvider_fiatRateUSD.decimals} USD)
"""

# ╔═╡ b016acf1-1fe9-4c50-8a72-04a87539173b
md"""
## 6.- Offline version.
...
# ╔═╡ 7481aecb-f31d-4f27-8d14-05c1b4c52a01
md"""
## 7.- Loading events from file.
"""

# ╔═╡ a60d6a6b-22dd-41f6-b831-7b0ed1c6d3f2
begin
	events = []
...
# ╔═╡ 21d3e099-bb63-436b-aff8-53fe3415466f
# Number of file events 
length(events)

# ╔═╡ 9140a230-dec6-42e4-b68a-39bfc2386f21
md"""
## 8.- Money Flow.
...
account nº5
"up" will add cumulatively the flows of 1+2+3+4+5
"down" will add cumulatevely the flows of 5+6+...

That will help to manipulate the parameters of the charts below without recalculation. 
"""

# ╔═╡ 5a33a02c-58b2-46c8-bd48-cf061acf43cc
begin
	prices = []
	cumUp = Dict()
	cumDown = Dict()
	...
	for event in events
	    ...
	    if evEthQ > 0  
			# transaccion > 0 eth. Another iteration will add acc with 0 to the filter
	        evTx = ev[10]
	        flowSign = 0
	        aRank = 0
...
# ╔═╡ 97d69034-49bb-4031-b236-c3a1873002a2
md"""
## 9.- Interactive Plots.

"""

# ╔═╡ 3327ac90-b65f-42a8-8b24-1751c0658442
md"""
##### a) Aggregation Down
...
# ╔═╡ 55c96f48-1845-4f37-966c-2053e08b0217
md"""
...
# ╔═╡ f9ff8672-6c6e-47e4-89e4-bf36a2fb0eac
@bind i html"<input type=range min=1 max=4505>"


# ╔═╡ 92459fbf-9e01-48b9-975f-2966f7d3f5d2
i

# ╔═╡ 436d1ea5-d4e4-49ea-8218-3afc224bd218
plot( 
	[(cumDown[i] .- mean(cumDown[i])) ./ std(cumDown[i]), (ethPrice .- mean(ethPrice)) ./ std(ethPrice)], label=["Normalized Money Flow" "Normalized ETH price"], lw = 3)

# ╔═╡ d50c00e0-89ee-4d80-8892-4f800caf303d
md"""
##### b) Aggregation Up
...
# ╔═╡ 77e5f838-1e73-4fd5-8de3-8e9626d024b5
md"""
###### Both time series are normalized to plot.
###### ---------------------------------------
...
# ╔═╡ 28706149-1eec-4378-ab9c-fcf1c133df3a
@bind j html"<input type=range min=1 max=4505>"

# ╔═╡ 9e266440-1770-4c86-bb4b-1013bde51bba
j

# ╔═╡ 0d14a39f-d361-4d9b-adb5-f7599a384f5d
plot( 
	[(cumUp[j] .- mean(cumUp[j])) ./ std(cumUp[j]), (ethPrice .- mean(ethPrice)) ./ std(ethPrice)], label=["Normalized Money Flow" "Normalized ETH price"], lw = 3 )
